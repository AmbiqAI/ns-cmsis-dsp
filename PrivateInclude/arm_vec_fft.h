...

{
    ...
    /* load scheduling to increase gather load idx update / gather load distance */
    bitRevTabOff = (uint32x4_t)vldrhq_u16(pBitRevTab);
    pBitRevTab += 8;

    bitRevOff0Low = (uint32x4_t)vmullbq_int_u16((uint16x8_t)bitRevTabOff, one);
    bitRevOff0High = (uint32x4_t)vmulltq_int_u16((uint16x8_t)bitRevTabOff, one);
    ...
    while (blkCnt > 0) {
        ...
        bitRevOff1Low = (uint32x4_t)vmullbq_int_u16((uint16x8_t)bitRevTabOff, one);
        bitRevOff1High = (uint32x4_t)vmulltq_int_u16((uint16x8_t)bitRevTabOff, one);
    ...
    if (blkCnt == 8) {
        ...
        bitRevTabOff = (uint32x4_t)vldrhq_z_u16(pBitRevTab, p);
